...
void UMainMenuWidget::NativeConstruct() {
	...
	if (GameInstance != nullptr) {
		...
		if (MyFirstGameLiftGameInstance != nullptr) {
			AccessToken = MyFirstGameLiftGameInstance->AccessToken;
		}
	}
			...
			if (JsonObject->HasField("access_token") && JsonObject->HasField("id_token") && JsonObject->HasField("refresh_token")) {
				...
				if (GameInstance != nullptr) {
					...
					if (MyFirstGameLiftGameInstance != nullptr) {
						...
						MyFirstGameLiftGameInstance->SetCognitoTokens(AccessToken, IdToken, RefreshToken);
...
void UMainMenuWidget::OnMatchmakingButtonClicked() {
	...
	if (GameInstance != nullptr) {
		...
		if (MyFirstGameLiftGameInstance != nullptr) {
			AccessToken = MyFirstGameLiftGameInstance->AccessToken;
			MatchmakingTicketId = MyFirstGameLiftGameInstance->MatchmakingTicketId;
		}
	}
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject)) {
			if (JsonObject->HasField("ticketId")) {
				...
				if (GameInstance != nullptr) {
					UMyFirstGameLiftGameInstance* MyFirstGameLiftGameInstance = Cast<UMyFirstGameLiftGameInstance>(GameInstance);
					if (MyFirstGameLiftGameInstance != nullptr) {
						MyFirstGameLiftGameInstance->MatchmakingTicketId = MatchmakingTicketId;
...
void UMainMenuWidget::OnStopMatchmakingResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful) {
	...
	if (GameInstance != nullptr) {
		UMyFirstGameLiftGameInstance* MyFirstGameLiftGameInstance = Cast<UMyFirstGameLiftGameInstance>(GameInstance);
		if (MyFirstGameLiftGameInstance != nullptr) {
			MyFirstGameLiftGameInstance->MatchmakingTicketId = "";
		}
	}
